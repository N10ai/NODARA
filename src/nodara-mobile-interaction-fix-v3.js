...
function markTransientOpen(){keepY=window.scrollY;suppressUntil=performance.now()+450}
...
document.addEventListener('focusin',e=>{
  ...
  requestAnimationFrame(()=>{try{el.blur()}catch{}window.scrollTo(0,keepY)})
},true);
...
function hud(text='Listening…',processing=false){
 ...
 const h=document.createElement('div');h.className=`nodara-voice-hud${processing?' processing':''}`;h.innerHTML=`<span class="wave"><i></i><i></i><i></i><i></i></span><span>${text}</span>${processing?'':'<button type="button">Stop</button>'}`;document.body.appendChild(h);h.querySelector('button')?.addEventListener('click',()=>window.nodaraStopVoice?.());return h
}
function clearHud(){document.querySelector('.nodara-voice-hud')?.remove();document.querySelectorAll('[data-nctx="voice"].listening').forEach(b=>b.classList.remove('listening'))}

/* Wrap the real voice runtime once it is available, preserving its backend transcription path. */
function wrapVoice(){
 const fn=window.nodaraVoiceCommand;
 if(!fn||fn.__nodaraVisualWrapped)return;
 const wrapped=async(...args)=>{
   const button=document.querySelector('.nodara-context-sheet [data-nctx="voice"]');button?.classList.add('listening');hud('Listening…');
...
/* Observe existing runtime toast text so HUD always reflects recording/transcription state. */
function syncState(){
 cleanMics();wrapVoice();
 const t=document.querySelector('.nodara-voice-toast');
 if(t){const txt=t.textContent||'';if(/Transcribing/i.test(txt))hud('Transcribing…',true);else if(/Listening/i.test(txt))hud('Listening…',false)}else if(document.querySelector('.nodara-voice-hud')&&!window.__nodaraVoiceActive)setTimeout(clearHud,120)
}
...
document.addEventListener('click',e=>{
 const v=e.target.closest?.('[data-nctx="voice"]');if(v){v.classList.add('listening');hud('Listening…')}
},true);

dezoom();cleanMics();setInterval(syncState,500);

...
async function mount(){
 ...
 const{data}=await supabase.from('shipments').select('mode').eq('id',c.id).maybeSingle();if(data?.mode!=='AIR')return;
 if(c.panel.dataset.sh33AirFor===c.id)return;busy=true;try{const x=await load(c.id);draw(c,x.s,x.link)}catch(e){console.warn('[NODARA] air execution',e)}finally{busy=false}
}

const css=document.createElement('style');css.textContent=`.sh33-air{display:grid;gap:10px}.sh33-head,.sh33-rolebar{display:flex;align-items:center;justify-content:space-between;gap:12px}.sh33-head h3{margin:2px 0}.sh33-head small,.sh33-rolebar span,.sh33-section-title small{color:var(--muted,#8c95a7);font-size:10px}.sh33-rolebar{padding:10px 11px;border:1px solid rgba(105,169,255,.16);border-radius:13px;background:rgba(70,137,245,.04)}.sh33-rolebar span,.sh33-rolebar b{display:block}.sh33-rolebar b{font-size:12px;margin-top:2px}.sh33-section{padding:11px;border:1px solid rgba(255,255,255,.07);border-radius:14px;background:rgba(255,255,255,.015)}.sh33-section-title{display:flex;justify-content:space-between;gap:10px;margin-bottom:9px}.sh33-section-title b{font-size:13px}.sh33-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.sh33-field{display:grid;gap:5px}.sh33-field span{font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted,#8c95a7);font-weight:700}.sh33-field input,.sh33-field select,.sh33-field textarea{width:100%;box-sizing:border-box;border:1px solid rgba(255,255,255,.09);border-radius:11px;background:rgba(0,0,0,.13);color:inherit;padding:9px 10px;font:inherit}.sh33-field input:disabled,.sh33-field select:disabled{opacity:.55}.sh33-span2{grid-column:span 2}.sh33-con-list{display:grid;gap:6px}.sh33-con-list button{display:grid;grid-template-columns:1fr auto;gap:2px 12px;text-align:left;padding:11px;border:1px solid rgba(255,255,255,.08);border-radius:12px;background:transparent;color:inherit}.sh33-con-list button small{grid-column:1/-1;color:var(--muted,#8c95a7)}@media(max-width:760px){.sh33-head,.sh33-rolebar{align-items:stretch;flex-direction:column}.sh33-head button,.sh33-rolebar button{width:100%}.sh33-grid{grid-template-columns:1fr 1fr}.sh33-span2{grid-column:1/-1}}@media(max-width:480px){.sh33-grid{grid-template-columns:1fr}.sh33-span2{grid-column:auto}}`;document.head.appendChild(css);
document.addEventListener('click',e=>{if(e.target.closest?.('[data-txw-tab="execution"]'))setTimeout(mount,0)},true);
new MutationObserver(()=>queueMicrotask(mount)).observe(main,{childList:true,subtree:true});

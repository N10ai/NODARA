...
async function detail(id){
 const {data:a,error}=await supabase.from('ftz_admission_workspace').select('*').eq('id',id).single();if(error)throw error;
 ...
 <div class="card"><h2>Admission lifecycle</h2><div class="ftz-flow"><b>Prepare</b><span>→</span><b>Authorize</b><span>→</span><b>Receive</b><span>→</span><b>Reconcile</b><span>→</span><b>Close</b></div><p class="muted">Next wiring: link an existing/new WR, compare expected vs received cargo, and post the reconciled cargo to this inventory identity.</p><button class="secondary" id="adm-list">Back to admissions</button></div>`;document.getElementById('adm-list').onclick=list;
}
window.nodaraFTZ={list,config:configScreen,newAdmission:createScreen,detail};

...
async function detail(id){
 const [{data:a,error},{data:wrs},{data:rec}]=await Promise.all([supabase.from('ftz_admission_workspace').select('*').eq('id',id).single(),supabase.from('ftz_admission_receipt_workspace').select('*').eq('admission_id',id),supabase.from('ftz_admission_reconciliation').select('*').eq('admission_id',id)]);if(error)throw error;

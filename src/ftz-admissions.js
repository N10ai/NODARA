...
function ftzBack(label='Back',handler=null){document.querySelector('.ftz-top-back')?.remove();const b=document.createElement('button');b.className='ftz-top-back';b.innerHTML='‹ <span>'+esc(label)+'</span>';b.onclick=handler||(()=>history.back());document.body.appendChild(b)}
function clearFtzBack(){document.querySelector('.ftz-top-back')?.remove()}
...
async function linkReceiptScreen(id){
 ...
 document.getElementById('ftz-link-back').onclick=()=>detail(id);document.getElementById('ftz-create-wr').onclick=()=>createExpectedReceipt(id);document.querySelectorAll('[data-wr]').forEach(b=>b.onclick=async()=>{const {error:e}=await supabase.rpc('nodara_link_ftz_receipt',{p_admission_id:id,p_warehouse_receipt_id:b.dataset.wr});if(e)throw e;await detail(id)});
}
...
async function detail(id){
 ...
 <div class="card"><div class="ftz-sectionhead"><div><div class="eyebrow">PRE-ALERT</div><h2>Expected cargo</h2><p class="muted">What should arrive. Receiving records what physically arrives; reconciliation compares the two.</p></div><button class="secondary" id="adm-add-expected">+ Cargo</button></div><div class="ftz-expected">${expected?.length?expected.map(x=>`<div class="ftz-cargo-line"><span>${x.line_no}</span><div><b>${esc(x.description||x.part_number||x.package_type||'Cargo')}</b><small>${esc([x.part_number,x.package_type].filter(Boolean).join(' · '))}</small></div><strong>${x.quantity} ${esc(x.uom)}</strong></div>`).join(''):'<div class="empty">No expected cargo yet. Add it manually or later extract it from the packing list / invoice.</div>'}</div></div><div class="card"><h2>Receiving records</h2>${wrs?.length?wrs.map(w=>`<div class="status"><button class="subtle" data-open-wr="${w.warehouse_receipt_id}">${esc(w.receipt_number)}</button><div><b>${w.receipt_status==='draft'?'EXPECTED':esc(w.receipt_status||'Linked')}</b> <button class="subtle" data-unlink-wr="${w.warehouse_receipt_id}">Unlink</button></div></div>`).join(''):'<div class="empty">No WR linked yet.</div>'}<div class="actions"><button class="primary" id="adm-create-wr">+ Create expected WR</button><button class="secondary" id="adm-link-wr">Link existing WR</button><button class="secondary" id="adm-reconcile" ${!wrs?.length?'disabled':''}>Reconcile receiving</button></div></div><div class="card"><h2>Inventory control posting</h2><div class="status"><span>Reconciliation lines</span><b>${rec?.length||0}</b></div><div class="status"><span>Open variances</span><b>${rec?.filter(x=>x.resolution_status==='OPEN').length||0}</b></div><p class="muted">Cargo is posted to the configured Zone Lot/UIN only after receiving reconciliation has no unresolved variance.</p><button class="primary wide" id="adm-post" ${!rec?.length||rec.some(x=>x.resolution_status==='OPEN')||a.admission_status==='CLOSED'?'disabled':''}>Post FTZ inventory & close admission</button></div><div class="card"><h2>Documents</h2><p class="muted">Case-file organization stays usable even when an admission accumulates dozens of 7512s, invoices and evidence files.</p><div class="ftz-doc-groups">${docs?.length?Object.entries(docs.reduce((g,d)=>((g[d.category]??=[]).push(d),g),{})).map(([cat,items])=>`<details class="ftz-doc-group" ${items.some(x=>x.is_key_document)?'open':''}><summary><b>${esc(cat.replaceAll('_',' '))}</b><span>${items.length}</span></summary>${items.map(d=>`<div class="ftz-doc-row"><div><b>${esc(d.display_name||d.file_name||d.document_type)}</b><small>${esc(d.document_number||d.record_type||'Supporting document')}</small></div><time>${d.document_date?new Date(d.document_date).toLocaleDateString():new Date(d.created_at).toLocaleDateString()}</time></div>`).join('')}</details>`).join(''):'<div class="empty">No documents linked to this admission yet.</div>'}</div></div><div class="card"><h2>Chronological activity</h2><p class="muted">Operational events and linked records are preserved in sequence.</p><div class="ftz-timeline">${timeline?.length?timeline.map(e=>`<div class="ftz-event"><time>${new Date(e.event_at).toLocaleString()}</time><div><b>${esc(e.title)}</b><small>${esc(e.detail||e.event_type)}</small></div></div>`).join(''):'<div class="empty">No activity yet.</div>'}</div></div><div class="card"><h2>Admission lifecycle</h2><div class="ftz-flow"><b>Prepare</b><span>→</span><b>Authorize</b><span>→</span><b>Receive</b><span>→</span><b>Reconcile</b><span>→</span><b>Close</b></div><button class="secondary" id="adm-list">Back to admissions</button></div>`;document.querySelectorAll('[data-open-wr]').forEach(b=>b.onclick=()=>window.nodaraOpenCanonicalWR?.(b.dataset.openWr));document.querySelectorAll('[data-unlink-wr]').forEach(b=>b.onclick=async()=>{if(!confirm('Unlink this WR from the FTZ Admission? The WR itself will remain.'))return;const {error}=await supabase.rpc('nodara_unlink_ftz_receipt',{p_admission_id:id,p_warehouse_receipt_id:b.dataset.unlinkWr});if(error)return alert(error.message);await detail(id)});document.getElementById('adm-add-expected')?.addEventListener('click',async()=>{const desc=prompt('Expected cargo description / part number');if(desc===null)return;const qty=prompt('Expected quantity');if(qty===null)return;const uom=prompt('Unit (CTN, PLT, EA, etc.)','CTN');if(uom===null)return;const {error}=await supabase.rpc('nodara_add_ftz_expected_cargo',{p_admission_id:id,p_description:desc,p_quantity:Number(qty),p_uom:uom,p_package_type:null,p_part_number:null});if(error)return alert(error.message);await detail(id)});document.getElementById('adm-delete')?.addEventListener('click',async()=>{if(!confirm('Delete Admission '+a.admission_number+'? Linked WRs will remain and become independent.'))return;const typed=prompt('Type Admission number '+a.admission_number+' to confirm deletion.');if(typed===null)return;const {error}=await supabase.rpc('nodara_delete_ftz_admission',{p_admission_id:id,p_confirmation:typed.trim()});if(error)return alert(error.message);await list()});document.getElementById('adm-list').onclick=list;document.getElementById('adm-create-wr').onclick=()=>createExpectedReceipt(id);document.getElementById('adm-link-wr').onclick=()=>linkReceiptScreen(id);document.getElementById('adm-reconcile').onclick=()=>reconcile(id);document.getElementById('adm-post').onclick=()=>postInventory(id);
}

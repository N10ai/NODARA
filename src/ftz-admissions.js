...
async function detail(id){
 const [{data:a,error},{data:wrs},{data:rec},{data:timeline},{data:audit}]=await Promise.all([supabase.from('ftz_admission_workspace').select('*').eq('id',id).single(),supabase.from('ftz_admission_receipt_workspace').select('*').eq('admission_id',id),supabase.from('ftz_admission_reconciliation').select('*').eq('admission_id',id),supabase.from('ftz_admission_timeline').select('*').eq('admission_id',id).order('event_at',{ascending:false}),supabase.from('ftz_audit_log').select('id,occurred_at,action,record_type,actor_id').eq('admission_id',id).order('occurred_at',{ascending:false}).limit(100)]);if(error)throw error;

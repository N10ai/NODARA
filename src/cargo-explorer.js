...
export function createCargoExplorer({ main, shell, esc, packageTypes }) {
  ...
  async function open(id){
    ...
    try{
      ...
      const m=meta(c),local=c.source_type==='TRANSACTION'&&m.transaction_local===true;\n      let location=null;if(c.source_type==='WAREHOUSE_RECEIPT'){const{data:cu}=await supabase.from('cargo_units').select('warehouse_location_id').eq('id',c.id).maybeSingle();if(cu?.warehouse_location_id){const{data:loc}=await supabase.from('warehouse_locations').select('id,code,name').eq('id',cu.warehouse_location_id).maybeSingle();location=loc||null}}
      ...
      <div class="grid2"><button class="actioncard" id="cargo-edit"><b>Edit cargo object</b><span>${local?'Edit this transaction-only cargo':'Edit descriptive cargo details'}</span></button>${c.source_type==='WAREHOUSE_RECEIPT'?'<button class="actioncard" id="cargo-source"><b>Open source WR</b><span>Open the receiving record</span></button>':''}</div>${c.source_type==='WAREHOUSE_RECEIPT'?`<div class="card"><div class="eyebrow">Put away</div><div class="field"><label>Warehouse location</label><input id="cargo-record-location" data-location-master autocomplete="off" placeholder="Search or scan location…" value="${esc(location?.code||'')}" data-location-id="${esc(location?.id||'')}"></div><p class="muted">Assign or move this cargo directly from the cargo record. Guided receiving is not required.</p><button class="primary wide" id="cargo-location-save">${location?'Move / update location':'Assign location'}</button><div id="cargo-location-msg"></div></div>`:''}
      ...
      document.getElementById('cargo-source')?.addEventListener('click',()=>{if(window.nodaraWROpen)return window.nodaraWROpen(c.source_id);alert('Open the source Warehouse Receipt from Warehouse Receipts.')});\n      document.getElementById('cargo-location-save')?.addEventListener('click',async()=>{const input=document.getElementById('cargo-record-location'),btn=document.getElementById('cargo-location-save'),out=document.getElementById('cargo-location-msg');const locId=input?.dataset.locationId||null;if(!locId){out.innerHTML='<p class="warning">Choose a saved warehouse location first.</p>';return}btn.disabled=true;btn.textContent='Saving…';const{error}=await supabase.from('cargo_units').update({warehouse_location_id:locId}).eq('id',c.id);btn.disabled=false;if(error){btn.textContent='Assign location';out.innerHTML=`<p class="warning">${esc(error.message)}</p>`;return}window.nodaraRefreshLocations?.();out.innerHTML='<p class="notice">Location assigned.</p>';setTimeout(()=>open(c.id),250)});

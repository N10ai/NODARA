...
export function openSystemSettings(initial='general'){
 ...
 function bindShell(){main.querySelectorAll('[data-sys-section]').forEach(b=>b.onclick=()=>{active=b.dataset.sysSection;render()});document.getElementById('sys-lists').onclick=()=>window.nodaraListsOptions?.();document.getElementById('sys-reset').onclick=()=>{if(confirm('Reset all system settings to NODARA defaults?')){state=clone(defaults);save(state);render()}}}

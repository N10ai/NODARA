...
export function createInventoryExplorer({ main, shell, esc, packageTypes }) {
  ...
  async function list() {
    window.nodaraSetActive?.('inventory');
    main.innerHTML='<div class="eyebrow">Warehouse · Inventory</div><h1 class="title">Inventory</h1><p class="muted">Loading live stock…</p>';
    try {
      const rows = await getInventorySnapshot();
      const totalItems=rows.length,onHand=rows.reduce((a,r)=>a+Number(r.on_hand||0),0),available=rows.reduce((a,r)=>a+Number(r.available||0),0),reserved=rows.reduce((a,r)=>a+Number(r.reserved||0),0),packages=rows.reduce((a,r)=>a+Number(r.package_count||0),0);
      main.innerHTML=`<div class="inventory-workspace-head"><div><div class="eyebrow">Warehouse · Inventory</div><h1 class="title">Inventory</h1><p class="muted">Live warehouse stock, physical packages, locations and allocations.</p></div><div class="inventory-switch"><button class="primary compact-btn">Stock</button><button class="secondary compact-btn" id="inventory-parts">Part Master</button></div></div><div class="inventory-kpi-grid"><div><span>Items</span><strong>${totalItems}</strong><small>with inventory records</small></div><div><span>On hand</span><strong>${onHand}</strong><small>total units</small></div><div><span>Available</span><strong>${available}</strong><small>ready to allocate</small></div><div><span>Allocated</span><strong>${reserved}</strong><small>reserved / committed</small></div><div><span>Packages</span><strong>${packages}</strong><small>physical handling units</small></div></div><div class="inventory-actionbar"><div><b>Current Stock</b><small>Search by part number, SKU, description or location.</small></div><div class="inventory-searchbox"><span>⌕</span><input id="inventory-filter" placeholder="Search stock…"></div></div><div id="inventory-list">${rows.length?renderList(rows):'<div class="empty"><b>No inventory yet.</b><small>Receive cargo or create part records to begin.</small></div>'}</div>`;
      document.getElementById('inventory-parts').onclick=()=>window.nodaraItemMaster?.();
      document.getElementById('inventory-filter').oninput = e => {
        const q = e.target.value.trim().toLowerCase();
        const filtered = !q ? rows : rows.filter(r => [r.item_key,r.sku,r.part_number,r.description,...(r.locations||[])].filter(Boolean).some(v=>String(v).toLowerCase().includes(q)));
        document.getElementById('inventory-list').innerHTML = filtered.length?renderList(filtered):'<div class="empty compact">No stock matches this search.</div>';
        bindList(filtered);
      };
      bindList(rows);
  ...
  function renderList(rows) {
    return `<div class="inventory-stock-list">${rows.map((r,i)=>`<button class="inventory-stock-row" data-item-index="${i}"><div class="inventory-stock-main"><div class="inventory-stock-icon">▦</div><div><b>${esc(r.part_number||r.sku||r.item_key||'Item')}</b><span>${esc(r.description||'No description')}</span><small>${r.locations?.length?esc(r.locations.join(' · ')):'No location assigned'}</small></div></div><div class="inventory-stock-numbers"><div><span>Available</span><strong>${Number(r.available||0)} ${esc(r.base_uom||'EA')}</strong></div><div><span>On hand</span><b>${Number(r.on_hand||0)}</b></div><div><span>Allocated</span><b>${Number(r.reserved||0)}</b></div><div><span>Packages</span><b>${Number(r.package_count||0)}</b></div></div><i>›</i></button>`).join('')}</div>`;
  }
  ...
  async function openItem(item) {
    ...
    try {
      ...
      shell('Inventory · Item',esc(item.part_number||item.sku||item.item_key),`<div class="record-commandbar"><button class="secondary compact-btn" id="inventory-back">‹ Inventory</button><button class="secondary compact-btn" id="inventory-master-open">Open Part Master</button></div><div class="inventory-item-summary"><div><span>Available</span><strong>${Number(item.available||0)} ${esc(item.base_uom||'EA')}</strong></div><div><span>On hand</span><strong>${Number(item.on_hand||0)}</strong></div><div><span>Allocated</span><strong>${Number(item.reserved||0)}</strong></div><div><span>Packages</span><strong>${Number(item.package_count||0)}</strong></div></div>${item.description?`<p class="muted">${esc(item.description)}</p>`:''}<div class="eyebrow" style="margin-top:26px">Physical cargo</div><div id="cargo-tree">${renderTree(currentRows)}</div>`);
      bindTree(); document.getElementById('inventory-back').onclick=list;document.getElementById('inventory-master-open').onclick=()=>window.nodaraItemMaster?.();

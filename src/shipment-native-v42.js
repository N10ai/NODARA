...
function clearNative(){
 ...
 delete main.dataset.wrNativeCanonical;
}
...
async function renderParties(shell,s){
 ...
 const tx=await transactionRow(s.id);if(!tx){panel.innerHTML='<div class="txw-empty">Shipment transaction registry is not ready yet.</div>';return}
 const[ents,ps]=await Promise.all([entityList(),partyRows(tx.id)]);

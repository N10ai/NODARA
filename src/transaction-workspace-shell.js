...
const ctxCache=new Map(),modelCache=new Map();
let busy=false,lastScreen='';
...
async function resolveContext(d){
  ...
  const{data,error}=await supabase.from(d.table).select(select).eq(d.numberField,d.number).maybeSingle();
  if(error)throw error;if(!data)return null;
  const ctx={...d,id:data.id,status:data.status,kind:d.type==='SHIPMENT'?`${label(data.mode)} Shipment`:d.type==='TRANSPORT_ORDER'?label(data.order_type):d.kind};
  ctxCache.set(d.screen,ctx);return ctx;
}
async function getModel(ctx,force=false){
  const key=`${ctx.type}:${ctx.id}`,hit=modelCache.get(key);
  if(!force&&hit&&Date.now()-hit.at<30000)return hit.model;
...
function execution(model,ctx){const m=nextMilestone(model),text=ctx.type==='WAREHOUSE_RECEIPT'?'Receiving, measurement, put-away and warehouse custody':ctx.type==='CARGO_RELEASE'?'Allocation, picking and physical release':ctx.type==='SHIPMENT'?'Routing, booking, carrier and forwarding execution':'Pickup, delivery, drayage and dispatch execution';return `<div class="txw-grid">${card('Execution type',ctx.kind)}${card('Status',ctx.status)}${card('Next operational time',m?fmt(m.milestone_at||m.milestone_date):'—')}${card('Workflow',(model?.requirements||[]).some(x=>x.blocking)?'Action required':'Clear')}</div><div class="txw-empty">${esc(text)} remains in the working controls below.</div>`}
const renderers={overview,parties,cargo,documents,charges,notes,activity};

function renderShell(ctx,model){
  const t=transactionCargoSummary(model),customer=primaryParty(model,'CUSTOMER'),ref=primaryReference(model),m=nextMilestone(model),shell=document.createElement('section');
...
function activatePrimarySurface(ctx){main.classList.add('txw-active');main.dataset.txwType=ctx.type}
function deactivate(){main.classList.remove('txw-active');delete main.dataset.txwType;main.querySelector('.txw-shell')?.remove();lastScreen=''}

async function enhance(force=false){
  if(busy||!main)return;
  const d=visibleDescriptor();
  if(!d){deactivate();return}
  if(!force&&d.screen===lastScreen&&main.querySelector('.txw-shell'))return;
  busy=true;
  try{
    const ctx=await resolveContext(d);if(!ctx)return;
    const model=await getModel(ctx,force);if(!model)return;
    main.querySelector('.txw-shell')?.remove();
    const shell=renderShell(ctx,model),anchor=main.querySelector('.record-commandbar,.context-bar,[data-wr-core] > .record-commandbar')||main.firstElementChild;
    if(anchor)anchor.insertAdjacentElement('afterend',shell);else main.prepend(shell);
    activatePrimarySurface(ctx);lastScreen=d.screen;
...
// Screen replacement changes direct children of #main. Nested editor/table mutations are ignored.
